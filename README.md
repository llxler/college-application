...
这是一个基于 `table.xlsx` 的本地 Streamlit 工具，用于按批次、科目、类别、学校所在省份、学校所在城市、分数、位次、专业关键词、学校性质和备注条件筛选院校专业组，并按“冲 / 稳 / 保”生成推荐结果。
...
3. 技能高考批需要选择技能类别。
4. 艺术批需要选择艺术类别。
5. 可搜索并多选学校所在省份和城市；城市选项会随已选省份联动，不选择时不过滤。当前批次没有省市数据时筛选框会禁用并提示。
6. 选择按分数或按位次录入，两种方式不会同时出现。
7. 普通批输入分数后，程序会根据 `rank.xlsx` 自动换算对应科目的位次，并按位次推荐。
8. 可选填写专业关键词，例如“计算机”“护理”“会计”“口腔医学”；多个关键词可用空格、逗号、顿号、分号、竖线或斜杠分隔，满足任意一个即保留。检索兼容全角/半角字符，英文不区分大小写。
9. 可选选择学校性质，例如公办、民办、中外合作等。
10. 可选设置备注排除项，例如中外合作办学、乡村振兴、专本联合培养、国家专项计划、护理类。
11. 左侧“显示设置”默认折叠；可按需选择结果表字段，删除列始终显示。默认显示推荐档位、院校专业组、院校专业组名称、首选科目、再选科目、位次值、专业信息和学校性质。
12. 设置每档推荐数量后查看结果表格。
13. 可勾选结果左侧的删除框，再点击“删除选中志愿”。
14. 选择 Excel、PDF 或 JPG 格式后下载；文件名会包含首选科目（或类别）和分数。
15. “位次推荐阈值”底部的说明中提供了计算方式、正负方向、填写格式和示例。

## 推荐规则

位次优先。普通批输入分数时，先使用 `rank.xlsx` 的累计人数换算位次；位次值越小，排名越靠前。
...
- 保留工作表中的“学校名称”“学校所在省份”和“学校所在城市”，用于动态省市筛选；缺少省市字段的数据保持为空，不虚构归属。
- 艺术本科表尾部空列会被忽略。
- `投档最低分` 或 `位次值` 缺失的数据标记为“数据不完整”，默认不参与推荐。
- 推荐结果保留原始备注。
- 导出结果仅保留“推荐档位”到“学校性质”的字段。
...
- 技能高考类别和艺术类别筛选。
- 体育批无专业字段时忽略专业关键词。
- 学校所在省份/城市联动筛选、学校性质、专业关键词和备注排除。
- 位次优先、分数-only 推荐、阈值边界。
- `rank.xlsx` 的物理/历史分数位次换算和缺档分数匹配。
- 缺失分数或位次的数据不进入推荐。
- Excel、PDF、JPG 导出文件有效性、精简字段和导出文件名。
